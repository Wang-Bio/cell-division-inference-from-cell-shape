...
public class CellDivisionInference implements PlugIn {
    ...
    private static class OuterDetectionConfig {
        ...
        // The consensus detector is authoritative by default.  Area error is
        // still evaluated for conservative redundancy pruning below.
        boolean enableAreaBasedVertexAddition=false;
        double maximumFitError=1.25, maxAreaErrorFraction=0.16, consensusRadius=4.0, junctionExclusion=3.0;
    ...
    static final class DebugTolerance {
        static final double COORDINATE=1e-6, STRICT_DISTANCE_PX=1.0, SHORT_LINE_PX=2.0;
        static final double BOUNDARY_DISTANCE_PX=2.0, MINIMUM_AREA_PX2=1.0;
        static final double RELATIVE_OVERLAP_AREA=1e-4, SUPPORTED_FRACTION=.80, MAX_UNSUPPORTED_GAP_PX=2.0;
    }
    enum DebugSeverity { ERROR, WARNING, INFO }
    static final class DebugIssue {
        final DebugSeverity severity; final String code,description,correction;
        final List<Integer> vertices,lines,polygons; final Rectangle2D.Double location;
        DebugIssue(DebugSeverity s,String c,List<Integer>v,List<Integer>l,List<Integer>p,Rectangle2D.Double r,String d,String f){severity=s;code=c;vertices=v;lines=l;polygons=p;location=r;description=d;correction=f;}
        String objects(){List<String>x=new ArrayList<>();for(int i:vertices)x.add("V"+i);for(int i:lines)x.add("E"+i);for(int i:polygons)x.add("P"+i);return String.join(", ",x);}
    }
    private Point2D.Double vertexGeometry(int i){if(i>=0&&i<vertexGeometryPoints.size())return vertexGeometryPoints.get(i);Point p=vertexPoints.get(i);return new Point2D.Double(p.x,p.y);}
    private List<Point2D.Double> debugPath(LineEdge e){if(e.pathValid&&e.path!=null&&e.path.size()>=2)return e.path;return Arrays.asList(vertexGeometry(e.v1),vertexGeometry(e.v2));}
    private static double debugCross(Point2D a,Point2D b,Point2D c){return(b.getX()-a.getX())*(c.getY()-a.getY())-(b.getY()-a.getY())*(c.getX()-a.getX());}
    private static boolean debugOn(Point2D p,Point2D a,Point2D b){return Math.abs(debugCross(a,b,p))<=DebugTolerance.COORDINATE&&p.getX()>=Math.min(a.getX(),b.getX())-1e-6&&p.getX()<=Math.max(a.getX(),b.getX())+1e-6&&p.getY()>=Math.min(a.getY(),b.getY())-1e-6&&p.getY()<=Math.max(a.getY(),b.getY())+1e-6;}
    private static int debugOrient(Point2D a,Point2D b,Point2D c){double x=debugCross(a,b,c);return x>1e-6?1:x<-1e-6?-1:0;}
    private static boolean debugIntersect(Point2D a,Point2D b,Point2D c,Point2D d){int x=debugOrient(a,b,c),y=debugOrient(a,b,d),z=debugOrient(c,d,a),w=debugOrient(c,d,b);return x*y<0&&z*w<0||x==0&&debugOn(c,a,b)||y==0&&debugOn(d,a,b)||z==0&&debugOn(a,c,d)||w==0&&debugOn(b,c,d);}
    private static double debugPointSegment(Point2D p,Point2D a,Point2D b){double dx=b.getX()-a.getX(),dy=b.getY()-a.getY(),q=dx*dx+dy*dy;if(q==0)return p.distance(a);double t=((p.getX()-a.getX())*dx+(p.getY()-a.getY())*dy)/q;t=Math.max(0,Math.min(1,t));return p.distance(a.getX()+t*dx,a.getY()+t*dy);}
    private static Rectangle2D.Double debugBox(Point2D a,Point2D b){return new Rectangle2D.Double(Math.min(a.getX(),b.getX()),Math.min(a.getY(),b.getY()),Math.abs(a.getX()-b.getX()),Math.abs(a.getY()-b.getY()));}
    private void issue(List<DebugIssue>r,DebugSeverity s,String c,List<Integer>v,List<Integer>l,List<Integer>p,Rectangle2D.Double b,String d,String f){r.add(new DebugIssue(s,c,v,l,p,b,d,f));}
    List<DebugIssue> inspectNetwork(){
        List<DebugIssue> r=new ArrayList<>();int n=vertexPoints.size();int[] degree=new int[n];for(LineEdge e:lineEdges){if(e.v1>=0&&e.v1<n)degree[e.v1]++;if(e.v2>=0&&e.v2<n)degree[e.v2]++;}
        for(int i=0;i<n;i++){Point2D a=vertexGeometry(i);if(degree[i]==0)issue(r,DebugSeverity.WARNING,"V004",Arrays.asList(i),Collections.emptyList(),Collections.emptyList(),debugBox(a,a),"Orphan vertex has degree zero.","Delete it or connect it to the intended wall.");else {VertexKind k=i<vertexKinds.size()?vertexKinds.get(i):VertexKind.MANUAL_TOPOLOGICAL;if(degree[i]==1||degree[i]>=4||degree[i]==2&&k!=VertexKind.CONTOUR_SUPPORT)issue(r,DebugSeverity.WARNING,"V005",Arrays.asList(i),Collections.emptyList(),Collections.emptyList(),debugBox(a,a),"Unexpected vertex degree "+degree[i]+".","Inspect the local topology; reconnect manually if incorrect.");}for(int j=i+1;j<n;j++){Point2D b=vertexGeometry(j);double d=a.distance(b);if(d<=1e-6)issue(r,DebugSeverity.ERROR,"V001",Arrays.asList(i,j),Collections.emptyList(),Collections.emptyList(),debugBox(a,b),"Distinct vertices occupy the same coordinate.","Merge or delete the duplicated vertex.");else if(d<=DebugTolerance.STRICT_DISTANCE_PX)issue(r,DebugSeverity.ERROR,"V002",Arrays.asList(i,j),Collections.emptyList(),Collections.emptyList(),debugBox(a,b),"Vertices are no more than one pixel apart.","Inspect the wall; merge/delete only if it is not biological.");}}
        for(int ei=0;ei<lineEdges.size();ei++){LineEdge e=lineEdges.get(ei);List<Point2D.Double> q=debugPath(e);Rectangle2D.Double box=debugBox(q.get(0),q.get(q.size()-1));if(!e.pathValid)issue(r,DebugSeverity.INFO,"E003",Collections.emptyList(),Arrays.asList(ei),Collections.emptyList(),box,"Legacy line has no stored wall path; displayed chord was used.","Re-detect or redraw the line to store its complete path.");if(e.v1==e.v2)issue(r,DebugSeverity.ERROR,"E001",Arrays.asList(e.v1),Arrays.asList(ei),Collections.emptyList(),box,"Line connects a vertex to itself.","Delete and reconnect the incorrect line.");double len=0;for(int k=1;k<q.size();k++)len+=q.get(k-1).distance(q.get(k));if(len<=2)issue(r,DebugSeverity.WARNING,"E010",Arrays.asList(e.v1,e.v2),Arrays.asList(ei),Collections.emptyList(),box,"Line is extremely short.","Verify image support before deleting or reconnecting it.");for(int a=0;a+1<q.size();a++)for(int b=a+2;b+1<q.size();b++)if(debugIntersect(q.get(a),q.get(a+1),q.get(b),q.get(b+1)))issue(r,DebugSeverity.ERROR,"E004",Collections.emptyList(),Arrays.asList(ei),Collections.emptyList(),box,"Line path intersects itself.","Delete and reconnect the incorrect line.");}
        for(int i=0;i<lineEdges.size();i++)for(int j=i+1;j<lineEdges.size();j++){LineEdge a=lineEdges.get(i),b=lineEdges.get(j);if(a.v1==b.v1&&a.v2==b.v2)issue(r,DebugSeverity.ERROR,"E002",Arrays.asList(a.v1,a.v2),Arrays.asList(i,j),Collections.emptyList(),debugBox(vertexGeometry(a.v1),vertexGeometry(a.v2)),"Duplicate lines have the same endpoints.","Delete the duplicated line after inspecting both paths.");boolean shared=a.v1==b.v1||a.v1==b.v2||a.v2==b.v1||a.v2==b.v2,hit=false;for(int x=0;x+1<debugPath(a).size()&&!hit;x++)for(int y=0;y+1<debugPath(b).size();y++)if(debugIntersect(debugPath(a).get(x),debugPath(a).get(x+1),debugPath(b).get(y),debugPath(b).get(y+1))){boolean endpoint=shared&&(debugPath(a).get(x).distance(debugPath(b).get(y))<1e-6||debugPath(a).get(x+1).distance(debugPath(b).get(y+1))<1e-6);if(!endpoint)hit=true;}if(hit)issue(r,DebugSeverity.ERROR,"E005",Collections.emptyList(),Arrays.asList(i,j),Collections.emptyList(),debugBox(vertexGeometry(a.v1),vertexGeometry(a.v2)),"Line interiors cross without a valid shared vertex.","Inspect both lines and delete/reconnect the incorrect one.");}
        for(int vi=0;vi<n;vi++)for(int ei=0;ei<lineEdges.size();ei++){LineEdge e=lineEdges.get(ei);if(vi==e.v1||vi==e.v2)continue;List<Point2D.Double>q=debugPath(e);for(int k=0;k+1<q.size();k++)if(debugPointSegment(vertexGeometry(vi),q.get(k),q.get(k+1))<=1e-6){issue(r,DebugSeverity.ERROR,"V003",Arrays.asList(vi),Arrays.asList(ei),Collections.emptyList(),debugBox(vertexGeometry(vi),vertexGeometry(vi)),"Vertex lies on the interior of an unrelated line.","Split the line at the vertex or delete and reconnect it.");break;}}
        for(int pi=0;pi<polygons.size();pi++){List<Integer> p=polygons.get(pi);Set<Integer>u=new HashSet<>(p);Rectangle2D.Double b=new Rectangle2D.Double();if(p.size()<3)issue(r,DebugSeverity.ERROR,"P001",Collections.emptyList(),Collections.emptyList(),Arrays.asList(pi),b,"Polygon boundary is not a closed cycle.","Rebuild the polygon from a closed sequence of graph lines.");double area=0;for(int k=0;k<p.size();k++){Point2D a=vertexGeometry(p.get(k)),c=vertexGeometry(p.get((k+1)%p.size()));area+=a.getX()*c.getY()-c.getX()*a.getY();}if(u.size()<3||Math.abs(area)/2<=1)issue(r,DebugSeverity.ERROR,"P002",Collections.emptyList(),Collections.emptyList(),Arrays.asList(pi),b,"Polygon is geometrically degenerate.","Delete it and reconnect its boundary before rebuilding polygons.");if(u.size()!=p.size())issue(r,DebugSeverity.ERROR,"P003",new ArrayList<>(p),Collections.emptyList(),Arrays.asList(pi),b,"Polygon repeats a nonclosing element.","Rebuild the polygon with each boundary element once.");}
    ...
    private static List<Point> detectOuterContourSupport(ByteProcessor bp,List<Point> anchors,OuterDetectionConfig cfg){
        ...
        TreeSet<Integer> cells=new TreeSet<>();for(AreaArc arc:areaArcs)cells.add(arc.cellLabel);for(int cell:cells){double before=areaError.applyAsDouble(cell),current=before;if(cfg.enableAreaBasedVertexAddition){while(current>cfg.maxAreaErrorFraction){AreaArc bestArc=null;int bestIndex=-1;double best=current;for(AreaArc arc:areaArcs)if(arc.cellLabel==cell)for(AreaCandidate candidate:arc.valid)if(!arc.selected.contains(candidate.index)){arc.selected.add(candidate.index);double trial=areaError.applyAsDouble(cell);arc.selected.remove(candidate.index);if(trial<best-1e-12){best=trial;bestArc=arc;bestIndex=candidate.index;}}if(bestArc==null)break;bestArc.selected.add(bestIndex);current=best;}
    ...
    private NeighborPairGeometryResult calculateGeometryForPair(int polyA, int polyB){
        ...
        // d_U = (A_hull,U - A_U) / ((A_1 + A_2) / 2).
        // For neighboring cells with disjoint interiors, the denominator also equals A_U / 2.
        double meanAreaOfTwoCells = 0.5 * (areaA + areaB);
        if(meanAreaOfTwoCells > 0.0 && Double.isFinite(unionArea) && !unionPoints.isEmpty()){
            ...
            if(hullArea > 0.0){
                ...
                double convexDeficiencyArea = Math.max(0.0, hullArea - unionArea);
                res.unionConvexDeficiency = convexDeficiencyArea / meanAreaOfTwoCells;
            }
        }
