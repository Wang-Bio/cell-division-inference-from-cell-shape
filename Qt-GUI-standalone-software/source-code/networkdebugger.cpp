...
QVector<NetworkDebugIssue> NetworkDebugger::inspect(QGraphicsScene *scene){
    ...
    for(int i=0;i<vs.size();++i){
        ...
        else if((deg==1||deg>=4||(deg==2&&vs[i]->kind()!=VertexItem::Kind::ContourSupport))) add(out,NetworkDebugIssue::Warning,"V005",{vs[i]->id()},{},{},QRectF(vs[i]->pos(),QSizeF()),"Unexpected vertex degree "+QString::number(deg)+".","Inspect the local topology; reconnect manually if incorrect.");
    ...
    for(auto*e:es){QVector<QPointF> q=path(e); QRectF box;for(auto p:q)box=box.united(QRectF(p,QSizeF()));if(!e->hasValidPath())add(out,NetworkDebugIssue::Info,"E003",{}, {e->id()},{},box,"Legacy line has no stored wall path; displayed chord was used.","Re-detect or redraw the line to store its complete path.");if(e->v1Id()==e->v2Id())add(out,NetworkDebugIssue::Error,"E001",{e->v1Id()},{e->id()},{},box,"Line connects a vertex to itself.","Delete and reconnect the incorrect line.");double len=0;for(int i=1;i<q.size();++i)len+=QLineF(q[i-1],q[i]).length();if(len<=NetworkDebugTolerances::shortLinePx)add(out,NetworkDebugIssue::Warning,"E010",{e->v1Id(),e->v2Id()},{e->id()},{},box,"Line is extremely short.","Verify image support before deleting or reconnecting it.");for(int i=0;i+1<q.size();++i)for(int j=i+2;j+1<q.size();++j)if(!(i==0&&j+1==q.size()-1)&&intersects(q[i],q[i+1],q[j],q[j+1]))add(out,NetworkDebugIssue::Error,"E004",{}, {e->id()},{},box,"Line path intersects itself.","Delete and reconnect the incorrect line.");}
